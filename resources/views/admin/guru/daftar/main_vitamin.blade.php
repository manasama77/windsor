<script>
    ...
    $(document).ready(function() {
        ...
        $('.datatables').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.guru.daftar.datatables') }}",
            columns: [{
                    data: 'id',
                    name: 'id'
                },
        ...
        $('body').on('click', '.delete', function() {
            if (confirm("Delete Record?") == true) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.guru.daftar.destroy') }}",
                    data: {
                        id: id,
                        _token: token,
                    },
    ...
    function editData(id) {
        idEdit = id
        $.ajax({
            url: `{{ url('admin/guru/daftar-guru/show') }}/${idEdit}`,
            method: 'get',
            dataType: 'json',
            beforeSend: e => {
                //
    ...
    function updateData() {
        $.ajax({
            url: `{{ url('admin/guru/daftar-guru/update') }}/${idEdit}`,
            method: 'post',
            dataType: 'json',
